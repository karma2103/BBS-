...
// Load the model
const File = require('../model/download');
const authController = require('./authController');

// @route GET /
// @desc Loads form and file list
app.get('/download', authController, async (req, res) => {
  try {
    const searchQuery = req.query.search || '';
    const regex = new RegExp(searchQuery, 'i'); 
    // Retrieve all files from the MongoDB database
    const files = await File.find().sort({ date: -1 });
    res.render('./download/download.ejs', { files, user : req.user });
  } catch (err) {
    res.status(500).send('An error occurred while loading the page');
  }
});

app.get("/Uploaddownload", authController, function (req, res) {
    res.render('./download/upload.ejs',  { user : req.user})
})
